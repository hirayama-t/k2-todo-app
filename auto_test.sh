...
# 2. テスト実行（XML出力）
npx jest src/testcase.test.js --ci --reporters=default --reporters=jest-junit --outputFile=testresult.xml

# 3. XML→CSVサマリー出力（簡易変換）
echo "ファイル,test名,結果" > testresult.csv
grep 'testcase.test.js' testresult.xml | sed -E 's/.*classname="([^"]+)" name="([^"]+)".*status="([^"]+)".*/\1,\2,\3/' | sed 's/\./,/g' >> testresult.csv
